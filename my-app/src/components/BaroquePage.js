...
const BaroquePage = () => {
  ...
  const [baroqueArtworks, setBaroqueArtworks] = useState([]);

  const title = 'Baroque';
  const description = "The Baroque art movement emerged in Europe during the 17th century, following the Renaissance period. Building upon the foundations laid by the Renaissance, Baroque art embraced a more dynamic and dramatic style, characterized by grandeur, opulence, and emotional intensity. It sought to evoke strong emotions and engage the viewer through elaborate compositions, rich ornamentation, and theatrical effects. Baroque art reflected the religious and political climate of the time, often commissioned by the Catholic Church and European monarchs. With its emphasis on dramatic contrasts and extravagant details, Baroque art paved the way for the subsequent Neoclassical movement, which sought to revive the aesthetics and principles of classical antiquity. The legacy of Baroque art can be seen in the grandeur and theatricality of Neoclassical works."
  ...
  useEffect(() => {
    const filterBaroqueArtworks = () => {
      const filteredArtworks = artworkData.filter((artwork) => artwork.era === title);
      setBaroqueArtworks(filteredArtworks);
    };

    filterBaroqueArtworks();
  }, [artworkData]);
  ...
  return (
    <Accordion defaultActiveKey="1" className='d-lg-none accordion-flush pt-5'>
      <Accordion.Item eventKey="0">
        ...
        <Accordion.Body>
        {baroqueArtworks.slice(0, 10).map((artwork) => (
...
{          `${description}`
  ...
  return (
    <div>
      <div className='baroquepage container min-vh-100 col-lg-10 col-sm-12 flex-column'>
